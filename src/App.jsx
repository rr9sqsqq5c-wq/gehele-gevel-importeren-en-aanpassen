...
function resolveCornerJoin(mainGroupId, secondaryGroupId, envelopeMap, getSettings, options = {}) {
  ...
  console.log(`[CJ] resolveCornerJoin ${mainGroupId}↔${secondaryGroupId} | front.side=${mainSide} front.extend=[str:${extendStrips} bat:${extendBattens}] | adj.side=${secSide} adj.extend=[str:${adjExtendStrips} bat:${adjExtendBattens}] | mainCorner=${mainCorner} secFarFace=${secFarFace} intersectStrips=${intersectionStrips} adjIntersect=${adjIntersectStrips}`);
  return join;
}
...
function applyCornerJoin(join, updateGroup, cornerConfigs, envelopeMap, getSettings) {
  ...
  const applyOneSide = (gid, side, strips, battens, panels) => {
    ...
    const eL = (patch.endExtensions.left?.strips ?? 0) > 0 ? patch.endExtensions.left.strips : 0;
    const eR = (patch.endExtensions.right?.strips ?? 0) > 0 ? patch.endExtensions.right.strips : 0;
    console.log(`[CJ] apply ${gid} zijde=${side} strips=${strips} bat=${battens} | extendLeft→rendering:${eL} extendRight→rendering:${eR} | oppSide(${oppSide})=${isOppSideActive ? 'BEWAARD' : 'GEWIST'}`);
    updateGroup(gid, patch);
  };
  console.log(`[CJ] applyCornerJoin START ${join.front.groupId}↔${join.adjacent.groupId}`);
