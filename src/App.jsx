...
import { scanIfcWallTypes, parseIfc, exportGroupsToIfc, warmupWebIFC, parseIfcGridLines, loadWalls } from './lib/ifc.js';
warmupWebIFC();
import { loadClashElements, detectClashesForGroup, buildClashExclusions, getGroupOuterFaceInfo } from './lib/clash.js';
...
export default function App() {
  ...
  const [clashExclusions, setClashExclusions] = useState({});
  const [clashLoadStatus, setClashLoadStatus] = useState('idle');
  ...
  async function startScan(file, handle) {
    ...
    try {
      addLog('IFC wanden laden via Agent 1 loader…');
      const walls = await loadWalls(file, (p) => {
        if (p.phase === 'init') {
          addLog(`${p.total} wanden gevonden, verwerken gestart…`);
        } else if (p.phase === 'wanden') {
          setLoadProgress({ current: p.current, total: p.total });
          if (p.current === p.total) addLog(`Alle ${p.total} wanden verwerkt`);
        }
      });

      if (!walls.length) throw new Error('Geen wanden gevonden in IFC-bestand');
      addLog(`✓ ${walls.length} wanden geladen`);
      ...
      try {
        const gl = await parseIfcGridLines(file);
        setGridLines(gl);
  ...
  async function confirmImport() {
    ...
    try {
      addLog('IFC wanden laden via Agent 1 loader…');
      const walls = await loadWalls(pendingFile, (p) => {
        if (p.phase === 'init') {
          addLog(`${p.total} wanden gevonden, verwerken gestart…`);
        } else if (p.phase === 'wanden') {
          setLoadProgress({ current: p.current, total: p.total });
  ...
  function runClashDetectionForAllGroups(currentGroups, currentWallMap, currentAllWalls, clashElements, currentPakketdikteTotal) {
    const newExclusions = {};
    for (const group of currentGroups) {
      const walls = group.wallIds.map(id => currentWallMap[id]).filter(Boolean);
      const faceInfo = getGroupOuterFaceInfo(group, currentWallMap, currentAllWalls);
      if (!faceInfo) continue;
      const clashes = detectClashesForGroup(
        group,
        walls,
        clashElements,
        currentPakketdikteTotal,
        faceInfo.thicknessAxis,
        faceInfo.outerFacePos,
        faceInfo.outDir,
      );
      if (clashes.length > 0) newExclusions[group.id] = clashes;
    }
    return newExclusions;
  }

  async function handleClashFileLoad(file) {
    setClashLoadStatus('loading');
    try {
      const elements = await loadClashElements(file, () => {});
      const newFile = { name: file.name, elements };
      const updatedClashFiles = [...clashFiles, newFile];
      setClashFiles(updatedClashFiles);

      const allClashElements = updatedClashFiles.flatMap(f => f.elements);
      const newExclusions = runClashDetectionForAllGroups(
        groups,
        wallMap,
        allWalls,
        allClashElements,
      ...
      setClashExclusions(newExclusions);
      setClashLoadStatus('loaded');
    } catch (err) {
      setClashLoadStatus('error');
    }
  }

  function applyClashExclusionsToGroup(groupId) {
    const clashes = clashExclusions[groupId];
    if (!clashes?.length) return;
    const s = getSettings(groupId);
    const exclusions = buildClashExclusions(clashes, s.zetwerk);
    const existingOpenings = s.clashOpenings ?? [];
    const newOpenings = exclusions.map(ex => ({
      x: ex.x,
      y: ex.y,
      breedte: ex.width,
      hoogte: ex.height,
    ...
    updateSettings(groupId, { clashOpenings: [...existingOpenings.filter(o => !o.isClash), ...newOpenings] });
  }

  function removeClashFile(fileName) {
    const updated = clashFiles.filter(f => f.name !== fileName);
    setClashFiles(updated);
    if (updated.length === 0) {
      setClashExclusions({});
    } else {
      const allClashElements = updated.flatMap(f => f.elements);
      const newExclusions = runClashDetectionForAllGroups(
        groups, wallMap, allWalls, allClashElements, pakketdikteTotal,
      );
      setClashExclusions(newExclusions);
    }
  }
  ...
  function autoGroup() {
    pushHistory(groups);
    _colorIdxRef.current = 0;
    const agent2Walls = wallsToGroupFormat(allWalls);
    const rawGroups = buildGroups(agent2Walls);
    const newGroups = rawGroups.map((g) => {
      const gid = newGid();
      const color = nextColor();
      initColor(gid, color, g.label);
      return { id: gid, wallIds: sortWallsInComponent(g.wallIds, allWalls, adjacencies) };
    });
    setGroups(newGroups);
    setSelectedWallIds(new Set());
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ background: '#1e293b', color: '#f8fafc', padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {allWalls.length > 0 && (
            <Tooltip text={"Laad een extra IFC-bestand (constructief model, architectuurmodel) om obstructies zoals balkons, luifels en kolommen te detecteren.\nGedetecteerde clashes worden als oranje zones getoond per gevelgroep."}>
              <label style={{ background: clashLoadStatus === 'loading' ? '#78350f' : '#b45309', color: '#fff', border: 'none', borderRadius: 4, padding: '4px 10px', fontSize: 12, cursor: clashLoadStatus === 'loading' ? 'default' : 'pointer', display: 'inline-block', whiteSpace: 'nowrap' }}>
                {clashLoadStatus === 'loading' ? '⏳ Laden…' : '⚠ Extra IFC laden'}
                <input
                  type="file"
                  accept=".ifc"
                  style={{ display: 'none' }}
                  disabled={clashLoadStatus === 'loading'}
                  onChange={(e) => {
                    const f = e.target.files[0];
                    if (f) { e.target.value = ''; handleClashFileLoad(f); }
                  }}
                />
              </label>
            </Tooltip>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
              ...
              {clashFiles.length > 0 && (
                <div style={{ flexShrink: 0, borderBottom: '1px solid #e2e8f0' }}>
                  <details open style={{ background: '#fff7ed' }}>
                    <summary style={{ padding: '6px 10px', fontSize: 11, fontWeight: 600, color: '#92400e', cursor: 'pointer', userSelect: 'none', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span>⚠ Clash-bestanden ({clashFiles.length})</span>
                      <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 700, color: '#92400e', background: '#fed7aa', borderRadius: 3, padding: '1px 6px' }}>
                        {Object.values(clashExclusions).reduce((s, v) => s + v.length, 0)} clashes
                      </span>
                    </summary>
                    <div style={{ padding: '4px 10px 8px' }}>
                      {clashFiles.map(cf => (
                        <div key={cf.name} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, marginBottom: 4 }}>
                          <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#78350f' }}>{cf.name}</span>
                          <span style={{ fontSize: 10, color: '#92400e', background: '#fed7aa', borderRadius: 8, padding: '1px 5px', flexShrink: 0 }}>{cf.elements.length} elem.</span>
                          <button
                            onClick={() => removeClashFile(cf.name)}
                    ...
                    return (
                      <div key={g.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                        ...
                        >
                          ...
                          {clashExclusions[g.id]?.length > 0 && (
                            <span title={`${clashExclusions[g.id].length} clash(es) gedetecteerd`} style={{ fontSize: 9, color: '#fff', background: '#f97316', borderRadius: 8, padding: '1px 5px', flexShrink: 0 }}>
                              ⚠ {clashExclusions[g.id].length}
                            </span>
                          )}
                        ...
                        {isActive && (
                          <div style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
                            ...
                            {clashExclusions[g.id]?.length > 0 && (
                              <div style={{ margin: '4px 10px', background: '#fff7ed', border: '1px solid #fdba74', borderRadius: 4, padding: '5px 8px' }}>
                                <div style={{ fontSize: 10, fontWeight: 600, color: '#92400e', marginBottom: 4 }}>
                                  ⚠ {clashExclusions[g.id].length} clash{clashExclusions[g.id].length !== 1 ? 'es' : ''} gedetecteerd
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginBottom: 6 }}>
                                  {clashExclusions[g.id].slice(0, 4).map((cl, ci) => (
                                    <div key={ci} style={{ fontSize: 10, color: '#78350f', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                      {cl.type} — {cl.name} ({Math.round(cl.depth)}mm diep)
                                    </div>
                                  ))}
                                  {clashExclusions[g.id].length > 4 && (
                                    <div style={{ fontSize: 10, color: '#94a3b8' }}>+{clashExclusions[g.id].length - 4} meer…</div>
                                  )}
                                </div>
                                <button
                                  onClick={() => applyClashExclusionsToGroup(g.id)}
                                  style={{ fontSize: 10, background: '#f97316', color: '#fff', border: 'none', borderRadius: 3, padding: '3px 8px', cursor: 'pointer', width: '100%' }}
                                >
                                  Pas zones aan (voeg toe als sparingen)
                                </button>
                              </div>
                            )}
                              ...
                              {[...groups].filter(og => og.id !== g.id).sort((a, b) => b.wallIds.length - a.wallIds.length).slice(0, 8).map(og => {
                                ...
                                return (
                                  <button key={og.id} onClick={() => {
                                    pushHistory(groups);
                                    const merged = mergeGroups(groups, g.id, og.id);
                                    const newId = merged[merged.length - 1].id;
                                    initColor(newId, os.color, `${s.name} + ${os.name}`);
                                    setGroups(merged);
                                    setActiveGroupId(newId);
                                  }} style={{ fontSize: 10, background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: 3, padding: '2px 6px', cursor: 'pointer' }}>
                                    + {os.name}
                                  </button>
                                );
                              })}
                              {g.wallIds.length > 1 && selectedWallIds.size > 0 && [...selectedWallIds].some(id => g.wallIds.includes(id)) && (
                                <button onClick={() => {
                                  pushHistory(groups);
                                  const idsToSplit = [...selectedWallIds].filter(id => g.wallIds.includes(id));
                                  const result = splitGroup(groups, g.id, idsToSplit);
                                  const newGroup = result[result.length - 1];
                                  initColor(newGroup.id, nextColor(), `Afgesplitst`);
                                  setGroups(result);
                                  setSelectedWallIds(new Set());
                                  setActiveGroupId(newGroup.id);
                                }} style={{ fontSize: 10, background: '#fef3c7', color: '#92400e', border: 'none', borderRadius: 3, padding: '2px 6px', cursor: 'pointer' }}>
                                  ✂ Splitsen ({[...selectedWallIds].filter(id => g.wallIds.includes(id)).length})
                                </button>
                              )}
              ...
              {activeGroup ? (
                <>
                  <View2D
                    ...
                    clashZones={clashExclusions[activeGroup.id] ?? []}
                  />
