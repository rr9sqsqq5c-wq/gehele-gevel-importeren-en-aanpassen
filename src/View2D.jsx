  ...
  useEffect(() => {
    ...
    if (clashZones.length > 0) {
      for (const cz of clashZones) {
        const [sx1c] = toScreen(cz.zoneX, 0);
        const [sx2c] = toScreen(cz.zoneX + cz.zoneWidth, 0);
        const [, sy1c] = toScreen(0, cz.zoneY);
        const [, sy2c] = toScreen(0, cz.zoneY + cz.zoneHeight);
        const czW = sx2c - sx1c;
        const czH = sy1c - sy2c;
        if (czW <= 0 || czH <= 0) continue;
        ctx.save();
        ctx.fillStyle = 'rgba(249,115,22,0.25)';
        ctx.fillRect(sx1c, sy2c, czW, czH);
        ctx.strokeStyle = 'rgba(249,115,22,0.85)';
        ctx.lineWidth = 2;
        ctx.setLineDash([6, 4]);
        ctx.strokeRect(sx1c, sy2c, czW, czH);
        ctx.setLineDash([]);
        ctx.fillStyle = 'rgba(249,115,22,0.9)';
        ctx.font = 'bold 9px system-ui, sans-serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        const czLabel = cz.name ? `⚠ ${cz.name}` : `⚠ ${cz.type}`;
        const czLw = Math.min(ctx.measureText(czLabel).width + 6, czW - 4);
        if (czLw > 10 && czH > 14) {
          ctx.fillRect(sx1c + czW / 2 - czLw / 2, sy2c + 2, czLw, 13);
          ctx.fillStyle = '#fff';
          ctx.fillText(czLabel, sx1c + czW / 2, sy2c + 3, czLw);
        }
        ctx.restore();
      }
    }
