...
function getWallRotationFromMesh(api, modelID, wID) {
  try {
    const mesh = api.GetFlatMesh(modelID, wID);
    if (!mesh || mesh.geometries.size() === 0) return 0;
...
function getWallOriginFromMesh(api, modelID, wID) {
  try {
    const mesh = api.GetFlatMesh(modelID, wID);
    if (!mesh || mesh.geometries.size() === 0) return null;
    const m = mesh.geometries.get(0).flatTransformation;
    return { x: Math.round(m[12] * 1000), y: Math.round(m[13] * 1000), z: Math.round(m[14] * 1000) };
  } catch {
    return null;
  }
}
...
export async function loadWalls(file, onProgress = null) {
  ...
  try {
    const wallStoreyMap = {};
    try {
      const relContVec = api.GetLineIDsWithType(modelID, IFC.IFCRELCONTAINEDINSPATIALSTRUCTURE);
      for (let i = 0; i < relContVec.size(); i++) {
        try {
          const rel = api.GetLine(modelID, relContVec.get(i), false);
          const storeyRef = rel?.RelatingStructure?.value;
          if (!storeyRef) continue;
          const rawStorey = api.GetRawLineData(modelID, storeyRef);
          if (rawStorey.type !== IFC.IFCBUILDINGSTOREY) continue;
          const storeyLine = api.GetLine(modelID, storeyRef, false);
          const storeyName = storeyLine?.Name?.value ?? `Verdieping #${storeyRef}`;
          const related = rel?.RelatedElements;
          if (!related) continue;
          for (let j = 0; j < related.length; j++) {
            const eid = related[j]?.value;
            if (eid) wallStoreyMap[eid] = storeyName;
          }
        } catch { }
      }
    } catch { }
    ...
    for (const wID of filteredWallIDs) {
      try {
        ...
        const thicknessAxis = sorted[0].axis;
        const heightAxis    = sorted[1].axis;
        const lengthAxis    = sorted[2].axis;

        const length    = Math.round(sorted[2].val * 1000);
        const height    = Math.round(sorted[1].val * 1000);
        const thickness = Math.round(sorted[0].val * 1000);

        if (length < 100 || height < 100) continue;

        const wallLine = api.GetLine(modelID, wID, false);
        const name = wallLine?.Name?.value ?? `Wand #${wID}`;
        const guid = wallLine?.GlobalId?.value ?? null;

        const rotation = getWallRotationFromMesh(api, modelID, wID);
        const meshOrigin = getWallOriginFromMesh(api, modelID, wID);

        const position = meshOrigin ?? {
          x: Math.round(bb[`min${lengthAxis.toUpperCase()}`] * 1000),
          y: Math.round(bb[`min${heightAxis.toUpperCase()}`] * 1000),
          z: Math.round(bb[`min${thicknessAxis.toUpperCase()}`] * 1000),
        };
        ...
        rawWalls.push({
          ...
          name,
          typeName: wallTypeMap[wID] ?? null,
          storey: wallStoreyMap[wID] ?? null,
          dimensions: { length, height, thickness },
          position,
          rotation,
          axes: { lengthAxis, heightAxis, thicknessAxis },
          bbox: {
            minX: Math.round(bb.minX * 1000),
            maxX: Math.round(bb.maxX * 1000),
            minY: Math.round(bb.minY * 1000),
            maxY: Math.round(bb.maxY * 1000),
            minZ: Math.round(bb.minZ * 1000),
            maxZ: Math.round(bb.maxZ * 1000),
          },
        });
      } catch { }
    ...
    return rawWalls.map(convertToAppWall);
  } finally {
    try { api.CloseModel(modelID); } catch { }
  }
}

export function convertToAppWall(w) {
  const { axes, bbox, dimensions } = w;
  const L = axes.lengthAxis.toUpperCase();
  const H = axes.heightAxis.toUpperCase();
  const T = axes.thicknessAxis.toUpperCase();
  return {
    expressID: w.expressID,
    guid: w.guid ?? null,
    name: w.name,
    typeName: w.typeName ?? null,
    ...
    wallOrigin: {
      ...
      thicknessAxis: axes.thicknessAxis,
      lengthStart: bbox[`min${L}`],
      heightStart: bbox[`min${H}`],
      thicknessStart: bbox[`min${T}`],
      thicknessEnd: bbox[`max${T}`],
    },
  };
}
